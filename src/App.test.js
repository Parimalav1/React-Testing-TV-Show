...
// mock "fetchMissions"
//jest.mock("./api/fetchShow"); // mock _ANY_ function coming from that file
...
test('data is fetched and rendered', async () => {
    ...
    axios.get=jest.fn().mockResolvedValueOnce(showData)
    // fetchShow.mockResolvedValueOnce(showData)
    const { queryByTestId, container, debug } = render(<App />);
    screen.debug();
    await waitFor(() => {
    ...
    // this is the element that opens the menu 👇
    // const dropdown = container.querySelector('.Dropdown-placeholder');
    // fireEvent.click(dropdown);
    
    // const dropdownMenu = container.querySelector('.Dropdown-menu'); // <-- this is the menu I expect to open (but it doesn't)
    // expect(dropdownMenu.children).toHaveLength(4);

        // screen.findByText(/Stranger Things/i)
});

test('Dropdown is fetched and rendered', async() => {
    await waitFor(() => {
        screen.findByText(/dropdown/i)
    })
});
