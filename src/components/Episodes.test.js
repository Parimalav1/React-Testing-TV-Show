...
test("Displays episodes as the episodes prop is updated", () => {
    ...
    //     // simulate that the episodes prop has changed and re-render the component
    //   //   screen.debug();

    //     // Make your assertion(s) for your test
});

test("Displays episodes as the episodes prop is updated", async() => {
    const { rerender } = render(<Episodes episodes={[showData.data._embedded.episodes]} />);
    rerender(<Episodes episodes={showData.data._embedded.episodes} />);
    screen.debug();
    await waitFor(() => {
        screen.findByText(/Chapter Eight/i);
    });
    expect(screen.getByText(/Chapter One: The Vanishing of Will Byers/i)).toBeInTheDocument();
  expect(screen.getAllByTestId("episodes")).toHaveLength(26);
});
